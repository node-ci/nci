...
Distributor.prototype._runNext = function(callback) {
	...
	Steppy(
		...
		function(err, build) {
			...
			// also add env vars specified for the project and node
			var executorParams = {
				envVars: _({
				...
				}).defaults(build.project.envVars, node.envVars)
			};
...
Distributor.prototype.run = function(params, callback) {
	...
	Steppy(
			...
			if (env) {
				...
				var funcs = _(project.envs).map(function(env) {
					return function() {
						...
						}).defaults(params), function(err, result) {
							if (err) {
								logger.error('Error during run: ', err.stack || err);
							} else if (result && result.builds) {
								groupRunBuilds = groupRunBuilds.concat(result.builds);
							}
							stepCallback();
						});
					};
		...
		function(err, build) {
			...
			self._runNext(function(err) {
				if (err) {
					logger.error(
						'Error during project run: ', err.stack || err
					);
				}
			});
		},
