...
Distributor.prototype._getNodeForChangesCheck = function(project, params) {
	return _(this.nodes).find(function(node) {
		return !node.getExecutorWaitReason(
			project,
			_({checkForChangesOnly: true}).defaults(params)
		);
	});
};
	...
	Steppy(
		function() {
			...
			if (params.withScmChangesOnly) {
				var node = self._getNodeForChangesCheck(project, params);
