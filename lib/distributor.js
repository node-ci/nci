...
Distributor.prototype._runNext = function(callback) {
	...
	Steppy(
		function() {
			...
			var reserveParams = {env: build.env};
			this.pass(reserveParams);

			// reserve executor must be in the first step (on the same step
			// that contains finding suitable node)
			executorId = node.reserveExecutor(build.project, reserveParams);
		...
		function(err, reserveParams, build) {
			this.pass(build);

			// release reserved executor coz we need params for envVars
			// that available only after build update(build number)
			node.releaseExecutor(executorId);

			_(reserveParams).extend({
				envVars: {
