...
Distributor.prototype._emitBuildUpdate = function(
	...
	this.emit('buildUpdated', eventParams);

	// for backward compatibility only
	// @TODO: remove on next major release
	this.emit('buildUpdate', build, changes);

	if (
		changes.status === 'in-progress' &&
		originalBuild.status !== 'in-progress'
	) {
		this.emit('buildStarted', eventParams);
	}

	if (changes.status && changes.status !== originalBuild.status) {
		this.emit('buildStatusChanged', eventParams);
	}

	if (changes.completed && !originalBuild.completed) {
		this.emit('buildCompleted', eventParams);
	}
};
