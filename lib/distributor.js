		...
		function() {
			...
			var queueItemIndex = _(self.queue).findIndex(function(item) {
				node = _(self.nodes).find(function(node) {
					return node.hasFreeExecutor(item.project, {env: item.build.env});
				});
				return node;
			});
			...
			var executor = node.run(build.project, {env: build.env}, function(err) {
				self._updateBuild(
					build,
					{
						...
						completed: true,
						error: err ? {message: err.message, stderr: err.stderr} : null
					},
					function(err, build) {
						if (err) {
							logger.error(
			...
			// executor may don't exist on error
			if (executor) {
...
Distributor.prototype._updateWaitReasons = function() {
	...
	_(self.queue).each(function(item) {
		var waitReasons = _(self.nodes).map(function(node) {
			return node.getExecutorWaitReason(item.project, {env: item.build.env});
		});
...
Distributor.prototype.run = function(params, callback) {
	...
	params.buildParams = params.buildParams || {};
