...
Executor.prototype.cancel = function(callback) {
	...
	Steppy(
		function() {
			if (!self.currentCmd) {
				throw new Error('Can`t canel build: no current cmd');
			}

			if (!self.currentCmd.pid) {
				throw new Error('Can`t canel build: current cmd doesn`t have pid');
			}

			// TODO: check that program is running before kill

			killTree(self.currentCmd.pid, 'SIGTERM', this.slot());
		},
		function() {
			self.canceled = true;
