...
function isScript(str){
	return (process.platform === 'win32' && str.endsWith('.bat')) || 
	(process.platform !== 'win32' && str.endsWith('.sh'));
}
...
Command.prototype.run = function(params, callback) {
	return ParentCommand.prototype.run.call(this, {
		cmd: this.shell,
		args: isScript(params.cmd) ? 
			this.shellExtraArgs.concat(params.cmd) : 
			this.shellExtraArgs.concat(this.shellCmdArg).concat(params.cmd),
